...
const QuestionPageInfo = ({ Questionreducer, FetchQuestion }) => {
  ...
  useEffect(() => {
    ...
    const getQuestionInfo = async () => {
      let Questiondata = await axios.get(
        `${API}/get/allposts/questionId/${id}`
      );
  ...
  const handletextarea = async (e) => {
    ...
    try {
      await axios.post(`${API}/post/answer/${id}`, {
  ...
  const handleAnswerDelete = async (answerId) => {
    ...
    if (isConfirm) {
      try {
        await axios.delete(
          `${API}/question/${id}/delete/${answerId}`,
          config
        );
        FetchQuestion();
  ...
  const handleSolvedAndUnsolved = async (status) => {
    try {
      await axios.put(`${API}/question/status/${id}/${status}`);
                  ...
                  ? filterdata.answers.map((answer) => {
                      return (
                        <Fragment key={answer._id}>
                          ...
                          <p className={css.answerquestion}>
